...
### Image delivery — planned migration to Cloudflare Image Transformations

**Status: Proposed. Blocked on the client's DNS move to Cloudflare + client sign-off.**
...
- Cropping becomes a URL param (`fit=cover` + `width`/`height` + `gravity`). Payload already stores `focalX`/`focalY` on every Media doc and the focal-point editor is already in the admin UI (currently a no-op) — pass `gravity=${focalX/100}x${focalY/100}` and it starts driving crops. This removes any need for named crop variants in the CMS. `gravity=auto` (saliency detection) is the fallback default. Both free.
- Same change is the natural place to add `loading="lazy"` (below-the-fold) / `fetchpriority="high"` (LCP image) to `MediaItem.astro`, which it currently lacks entirely.
...
1. Prereq: R2 bucket on a custom domain on a Cloudflare zone; enable Images → Transformations on that zone.
2. `cms/src/collections/Media.ts`: drop `imageSizes` (optionally keep one small `adminThumbnail` size for admin-UI performance); keep `focalX`/`focalY`. `sharp` stays a dependency — Payload requires it.
3. New helper `web/src/lib/image.ts`: `cfImage(src, { width, height?, fit?, gravity?, quality })` → builds the `/cdn-cgi/image/…` URL from a configurable base.
4. Rewrite the `srcset` builder in `web/src/components/MediaItem.astro`: short fixed width ladder (~400 / 800 / 1200 / 1600 / 2000); fixed-ratio contexts (the existing `aspectRatio` prop) emit `fit=cover` + computed `height`; fluid contexts emit width only; `gravity` from the stored focal point, else `auto`; `format=auto`; `quality` ~72. Add `loading` / `fetchpriority` via a new `priority` prop, threaded from the hero / LCP callers.
5. Bound the stored original to ~2560–3000px; keep `media.url` (original) as the `<img src>` fallback and the OG image.
6. After verifying the new build, delete the orphaned `-WxH.webp` variants from the R2 bucket.
...
| D-7 | Image delivery: migrate from the Sharp size ladder to Cloudflare Image Transformations (`format=auto`, on-the-fly resize/crop). Blocked on the client's DNS move to Cloudflare — see "Image delivery — planned migration to Cloudflare Image Transformations" | Proposed | 2026-09-07 | Team |
...
- Image delivery may move from the current Sharp/R2 `srcset` pipeline to Cloudflare Image
  Transformations — see "Image delivery — planned migration to Cloudflare Image Transformations"
  under Architecture decisions (D-7). Blocked on the same client Cloudflare account + DNS move to
  Cloudflare as Turnstile above.

## Performance and caching targets (draft)

- Mobile LCP target (p75): <= 2.5s
- Optimised responsive media across media-heavy blocks — currently the Sharp/R2/`srcset` pipeline (see "Images and responsive delivery" in `web/CLAUDE.md` and the `sizes`-attribute convention); a migration to Cloudflare Image Transformations is proposed (D-7)
